...
const FACTORY_DEFAULT = {
  ...
  // ── Premium ──
  premiumMatchMode: 'any',        // any | total | od | tp
  ...
  // ── Policy Trimming ──
  stmtTrimMode:     'none',       // none | first | last
  stmtTrimDigits:   20,
  masterTrimMode:   'none',
  masterTrimDigits: 20,
...
/* ── Policy trimming helper ── */
function trimPolicy(raw, mode='none', digits=20){
  const s = String(raw || '').replace(/[^a-zA-Z0-9]/g,'');
  const n = parseInt(digits) || 20;

  if(mode === 'first') return s.slice(0, n);
  if(mode === 'last') return s.slice(-n);
...
function premiumMatch(mRow, sOD, sTP, sTotal, tolPct, tolAbs, useShare, mode='any'){
  ...
  if(mode==='total'){
    hit = close(mTot,toNum(sTotal));
  }else if(mode==='od'){
    hit = close(mOD,toNum(sOD));
  }else if(mode==='tp'){
    hit = close(mTP,toNum(sTP));
  }else{
    hit = close(mOD,toNum(sOD)) ||
          close(mTP,toNum(sTP)) ||
          close(mTot,toNum(sTotal));
  }
...
function precomputeMaster(){
  masterData.forEach(r=>{
    ...
    r._trimmedPol = trimPolicy(
      r._freshPol,
      defaultRule.masterTrimMode || 'none',
      defaultRule.masterTrimDigits || 20
    );
...
function saveDefaultRuleFromForm(){
  ...
  defaultRule = {
    ...
    premiumMatchMode:  g('dr-prem-mode'),
    stmtTrimMode:      g('dr-stmt-trim-mode'),
    stmtTrimDigits:    g('dr-stmt-trim-digits','int'),
    masterTrimMode:    g('dr-master-trim-mode'),
    masterTrimDigits:  g('dr-master-trim-digits','int'),
...
function renderDefaultRuleForm(){
  ...
  s('dr-prem-mode',          defaultRule.premiumMatchMode || 'any');
  s('dr-stmt-trim-mode',     defaultRule.stmtTrimMode || 'none');
  s('dr-stmt-trim-digits',   defaultRule.stmtTrimDigits || 20);
  s('dr-master-trim-mode',   defaultRule.masterTrimMode || 'none');
  s('dr-master-trim-digits', defaultRule.masterTrimDigits || 20);
...
function buildStmtKeys(sRow){
  ...
  const effectiveRule = getEffectiveRule(String(sRow[stmtMapping.insurer]||''));

  const trimmedStmtPol = trimPolicy(
    fp,
    effectiveRule.stmtTrimMode || 'none',
    effectiveRule.stmtTrimDigits || 20
  );
...
/* ── Editable matching waterfall priority ── */
const MATCH_PRIORITY_DEFAULT = [
  'polno+endono+premium+name+date',
  'polno+premium+name',
  'endono+premium+name',
  'polno+premium',
  'name+premium',
...
function matchOneRow(sRow){
  ...
  // ── 2. Contra: policy total commission = 0 ──
...
if(sk.fp && stmtData){
    ...
    if(grp.length > 1){
      ...
      if(Math.abs(sumC) < 1){
        return mk(sRow, null, 45, 'partial', 'Contra', sIns);
      }
    }
  }
